...
if ($action === "create") {
  try {
    ...
    $asset_type_count = $ASSET->asset_type_count([$name]);

    if (intval($asset_type_count) > 0) {
      $VALIDATION->alert("danger", "ข้อมูลซ้ำในระบบ!", "/asset-type");
    }

    $ASSET->asset_type_insert([$name]);
...
if ($action === "update") {
  try {
    ...
    $ASSET->asset_type_update([$name, $status, $id]);
    $VALIDATION->alert("success", "ดำเนินการเรียบร้อย!", "/asset-type/view/{$id}");
  } catch (PDOException $e) {
    die($e->getMessage());
  }
}
