...
if ($action === "create") {
  try {
    ...
    $purchase_count = $PURCHASE->purchase_count([$login_id, $department, $date, $order_number, $objective]);
    if (intval($purchase_count) === 0) {
      $PURCHASE->purchase_insert([$purchase_last, $login_id, $department, $date, $order_number, $objective]);
...
if ($action === "update") {
  try {
    ...
    $order_number = (isset($_POST['order_number']) ? $VALIDATION->input($_POST['order_number']) : "");
    $objective = (isset($_POST['objective']) ? $VALIDATION->input($_POST['objective']) : "");
    $PURCHASE->purchase_update([$department, $date, $order_number, $objective, $uuid]);
