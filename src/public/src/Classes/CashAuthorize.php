...
class CashAuthorize
{
  ...
  {
    $sql = "SELECT COUNT(*) FROM belink.cash_authorize WHERE status = 1 AND type_id = ? AND login_id = ?";
  ...
  {
    $sql = "INSERT INTO belink.cash_authorize(`type_id`, `login_id`) VALUES(?,?)";
    $stmt = $this->dbcon->prepare($sql);
  ...
  {
    $sql = "UPDATE belink.cash_authorize SET
    status = 2,
    updated = NOW()
    WHERE id = ?";
    $stmt = $this->dbcon->prepare($sql);
  ...
  {
    $sql = "SELECT COUNT(*) FROM belink.cash_authorize WHERE `type_id` = ? AND login_id = ? AND status = 1";
    $stmt = $this->dbcon->prepare($sql);
    $stmt->execute($data);
    return $stmt->fetchColumn();
  }
  ...
  {
    $sql = "SELECT amount FROM belink.cash_money WHERE id = 1";
    $stmt = $this->dbcon->prepare($sql);
    $stmt->execute();
  ...
  {
    $sql = "UPDATE belink.cash_money SET
    amount = ?
    WHERE id = 1";
    $stmt = $this->dbcon->prepare($sql);
    return $stmt->execute($data);
  ...
  {
    $sql = "SELECT COUNT(*) FROM belink.cash_authorize WHERE status = 1";
    $stmt = $this->dbcon->prepare($sql);
    $stmt->execute();
