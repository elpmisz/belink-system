...
class Customer
{
  ...
  {
    $sql = "INSERT INTO belink.customer(`uuid`, `code`, `name`, `contact`) VALUES(uuid(),?,?,?)";
    $stmt = $this->dbcon->prepare($sql);
  ...
  {
    $sql = "UPDATE belink.customer SET
    code = ?,
    name = ?,
    contact = ?,
    status = ?,
  ...
  {
    $sql = "SELECT COUNT(*) FROM belink.customer a WHERE a.status IN (1,2)";
    $stmt = $this->dbcon->prepare($sql);
    $stmt->execute();
    $total = $stmt->fetchColumn();

    $column = ["a.id", "a.code", "a.name", "a.contact"];
    ...
    foreach ($result as $row) {
      $action = "<a href='/customer/view/{$row['uuid']}' class='badge badge-{$row['status_color']} font-weight-light'>{$row['status_name']}</a>";
