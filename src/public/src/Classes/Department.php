...
class Department
{
  ...
  {
    $sql = "INSERT INTO belink.department( `name`) VALUES(?)";
    $stmt = $this->dbcon->prepare($sql);
  ...
  {
    $sql = "UPDATE belink.department SET
    name = ?,
    status = ?,
    updated = NOW()
    WHERE id = ?";
  ...
  {
    $sql = "SELECT COUNT(*) FROM belink.department a WHERE a.status IN (1,2)";
    $stmt = $this->dbcon->prepare($sql);
    $stmt->execute();
    ...
    foreach ($result as $row) {
      $action = "<a href='/department/view/{$row['id']}' class='badge badge-{$row['status_color']} font-weight-light'>{$row['status_name']}</a>";
